...
@Configuration
@ConditionalOnProperty(name = "spring.batch.job.names", havingValue = "gs")
@RequiredArgsConstructor
public class Gs25BatchConfig {
    ...
    public Job createJob() {
        return jobBuilderFactory.get("gs")
                .start(createStep())
                .build();
    }
    ...
    public CvsCrawResultReader crawResultReader() {
        return new CvsCrawResultReader(new Gs25CrawlerClient(), resultConverter());
    }

    @Bean
    public ResultConverter resultConverter() {
        return new Gs25CrawlerResultConverter();
    }
